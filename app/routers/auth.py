...
def signup(user: UserCreate, db: mysql.connector.connection.MySQLConnection = Depends(get_db_connection)):
    ...
    cursor.execute(query, (user.Email, user.PhoneNumber))
    if cursor.fetchone():
        cursor.close()
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User already exists")
    
    hashes_password = get_password_hash(user.Password)
    insert_query = "INSERT INTO User (Email, PhoneNumber, Password, Role) VALUES (%s, %s, %s, %s)"
    cursor.execute(insert_query, (user.Email, user.PhoneNumber, hashes_password, "normalUser"))
    db.commit()
    cursor.close()
...
def send_otp(otp_request: OTPRequest, db: mysql.connector.connection.MySQLConnection = Depends(get_db_connection)):
    cursor = db.cursor()
    query = "SELECT UserID FROM User WHERE Email = %s OR PhoneNumber = %s"
    cursor.execute(query, (otp_request.req.phone_or_email, otp_request.req.phone_or_email))
...
def otp_login(otp_request: OTPLoginRequest, db: mysql.connector.connection.MySQLConnection = Depends(get_db_connection)):
    ...
    if not stored_otp or stored_otp.decode() != otp_request.OTP:
